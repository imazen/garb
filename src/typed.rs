...
//! typed::rgb_to_bgra_buf(&rgb_pixels, &mut bgra_buf);
//! ```
...
pub fn rgb_to_bgr_mut(pixels: &mut [Rgb<u8>]) -> &mut [Bgr<u8>] {
    let bytes: &mut [u8] = bytemuck::cast_slice_mut(pixels);
    crate::swap_rb_3bpp_inplace(bytes);
    bytemuck::cast_slice_mut(bytes)
}
...
pub fn rgba_to_bgra_buf(src: &[Rgba<u8>], dst: &mut [Bgra<u8>]) {
...
pub fn bgra_to_rgba_buf(src: &[Bgra<u8>], dst: &mut [Rgba<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
    crate::copy_swap_br(src_bytes, dst_bytes);
}
...
pub fn rgb_to_bgra_buf(src: &[Rgb<u8>], dst: &mut [Bgra<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
...
pub fn rgb_to_rgba_buf(src: &[Rgb<u8>], dst: &mut [Rgba<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
...
pub fn bgr_to_rgba_buf(src: &[Bgr<u8>], dst: &mut [Rgba<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
    crate::rgb_to_bgra(src_bytes, dst_bytes);
}
...
pub fn bgr_to_bgra_buf(src: &[Bgr<u8>], dst: &mut [Bgra<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
    crate::rgb_to_rgba(src_bytes, dst_bytes);
}
...
pub fn gray_to_rgba_buf(src: &[Gray<u8>], dst: &mut [Rgba<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
...
pub fn gray_to_bgra_buf(src: &[Gray<u8>], dst: &mut [Bgra<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
    crate::gray_to_4bpp(src_bytes, dst_bytes);
}
...
pub fn gray_alpha_to_rgba_buf(src: &[GrayAlpha<u8>], dst: &mut [Rgba<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
...
pub fn gray_alpha_to_bgra_buf(src: &[GrayAlpha<u8>], dst: &mut [Bgra<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
    crate::gray_alpha_to_4bpp(src_bytes, dst_bytes);
}
...
pub fn rgba_to_rgb_buf(src: &[Rgba<u8>], dst: &mut [Rgb<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
...
pub fn bgra_to_bgr_buf(src: &[Bgra<u8>], dst: &mut [Bgr<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
    crate::drop_alpha_4to3(src_bytes, dst_bytes);
}
...
pub fn bgra_to_rgb_buf(src: &[Bgra<u8>], dst: &mut [Rgb<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
...
pub fn rgba_to_bgr_buf(src: &[Rgba<u8>], dst: &mut [Bgr<u8>]) {
    let src_bytes: &[u8] = bytemuck::cast_slice(src);
    let dst_bytes: &mut [u8] = bytemuck::cast_slice_mut(dst);
    crate::drop_alpha_swap_4to3(src_bytes, dst_bytes);
}
...
mod tests {
    ...
    fn test_rgba_to_bgra_inplace() {
        ...
        assert_eq!(bgra[0], Bgra { b: 0, g: 128, r: 255, a: 200 });
        assert_eq!(bgra[1], Bgra { b: 30, g: 20, r: 10, a: 40 });
    }

    #[test]
    fn test_bgra_to_rgba_inplace() {
        let mut pixels = vec![Bgra { b: 0u8, g: 128, r: 255, a: 200 }];
    ...
    fn test_rgb_to_bgra_copy() {
        ...
        rgb_to_bgra_buf(&src, &mut dst);
        assert_eq!(dst[0], Bgra { b: 0, g: 128, r: 255, a: 255 });
    }
    ...
    fn test_rgb_to_rgba_copy() {
        ...
        rgb_to_rgba_buf(&src, &mut dst);
        assert_eq!(dst[0], Rgba::new(255, 128, 0, 255));
    }

    #[test]
    fn test_bgr_to_rgba_copy() {
        let src = vec![Bgr { b: 0u8, g: 128, r: 255 }];
        let mut dst = vec![Rgba::default(); 1];
        bgr_to_rgba_buf(&src, &mut dst);
        assert_eq!(dst[0], Rgba::new(255, 128, 0, 255));
    }
    ...
    fn test_gray_alpha_to_rgba() {
        ...
        let mut dst = vec![Rgba::default(); 1];
        gray_alpha_to_rgba_buf(&src, &mut dst);
    ...
    fn test_rgba_to_rgb_drop_alpha() {
        ...
        rgba_to_rgb_buf(&src, &mut dst);
        assert_eq!(dst[0], Rgb::new(255, 128, 0));
    }

    #[test]
    fn test_bgra_to_rgb_drop_and_swap() {
        let src = vec![Bgra { b: 0u8, g: 128, r: 255, a: 200 }];
        let mut dst = vec![Rgb::default(); 1];
        bgra_to_rgb_buf(&src, &mut dst);
        assert_eq!(dst[0], Rgb::new(255, 128, 0));
    }
    ...
    fn test_rgb_bgr_roundtrip() {
        ...
        assert_eq!(bgr[0], Bgr { b: 30, g: 20, r: 10 });
